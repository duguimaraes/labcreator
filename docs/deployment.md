...
git remote add origin https://github.com/yourusername/labcreator.git
...
\`\`\`bash
# Build image
docker build -t labcreator .

# Run container
docker run -p 3000:3000 labcreator
\`\`\`
...
\`\`\`bash
cd /var/www
sudo git clone https://github.com/yourusername/labcreator.git
cd labcreator
\`\`\`
...
# Start app
pm2 start pnpm --name "labcreator" -- start
...
# Create config
sudo nano /etc/nginx/sites-available/labcreator
\`\`\`
...
\`\`\`bash
# Enable site
sudo ln -s /etc/nginx/sites-available/labcreator /etc/nginx/sites-enabled/
...
location /_next/static {
    alias /var/www/labcreator/.next/static;
    expires 365d;
    add_header Cache-Control "public, immutable";
}

location /static {
    alias /var/www/labcreator/public;
    expires 365d;
    add_header Cache-Control "public, immutable";
}
\`\`\`
...
0 2 * * * /usr/bin/pg_dump labcreator > /backups/labcreator-$(date +\%Y\%m\%d).sql
\`\`\`
...
rsync -avz /var/www/labcreator/public/ backup-server:/backups/labcreator-public/
\`\`\`
